...
('don','don@gmail.com','123');

INSERT INTO USUARIO (NOME_USUARIO, EMAIL_USUARIO, SENHA_USUARIO) VALUES
('TESTE','TESTE@gmail.com','321');
...
INSERT INTO DADOS (FK_USUARIO,FK_ARCANOS) VALUES (${user},${sorteada})

-- COLOCAR NA METRICA CARTAS TIRADAS
SELECT COUNT(FK_ARCANOS) FROM DADOS WHERE FK_USUARIO = ${user};

-- COLOCAR NA DIVISÃO POR NAIPE
SELECT ARCANOS.NAIPE_ARCANO FROM DADOS JOIN ARCANOS ON NUMERO_ARCANO = FK_ARCANOS JOIN USUARIO ON ID_USUARIO = FK_USUARIO WHERE FK_USUARIO = ${user};

-- PEGANDO A CARTA QUE MAIS SAIU
SELECT FK_ARCANOS FROM DADOS WHERE FK_USUARIO = ${ID_USER} GROUP BY FK_ARCANOS ORDER BY COUNT(FK_ARCANOS) DESC LIMIT 1;

-- PEGANDO AS KEYWORDS
SELECT KEY1,KEY2,KEY3 FROM KEYWORDS WHERE FK_ARCANOS = ${sorteada};
